...
def para_tranz(list, key, original, translation="", context=""):
    if len(original) > 1 and original not in ["_REUSE"]:
...
def export_json(file_path):
    ...
    with open_n_decode(file_path, "r", "utf_8_sig") as f:
        ...
        if basename(file_path) == "Associations.json":
            ...
            for i in jsondata:
                if i["Tag"] in ['Ship Equipment Slot', 'Ship Conditions', 'ship conditions', 'EnemyStats']:
                    continue
                if i["Name"] != None:
                    list = para_tranz(list,
                                      str(i["Id"])+"$Name", i["Name"])
                if i["Description"] != None:
                    list = para_tranz(list,
                                      str(i["Id"])+"$Description", i["Description"])
                if i["AvailableAt"] != None:
                    list = para_tranz(list,
                                      str(i["Id"])+"$AvailableAt", i["AvailableAt"])
                    ...
                    if exists(export_file):
                        ...
                        if olddata == result:
                            ...
                            for newdata in range(len(result)):
                                for i in olddata:
                                    if i["key"] == result[newdata]["key"] and i["original"].replace("\\n","\n") == result[newdata]["original"].replace("\\n","\n") and i["translation"] != None:
